 ...
 */
const { findPath, checkIfExist, gFile, prompt, appendToFile, formatFile } = require('@rigel/utils');
...
exports.addApi = async (name, options) => {
	for (let i = 0; i < options.exports.length; i++) {
		...
		if (typeAnswer === 'Async/Await') {
			...
			gFile({ path: `${path}`, name: options.exports[i], type: 'api', content });
