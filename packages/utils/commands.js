 ...
 */
exports.cloneAndRenameApp = async (name) => {
	return new Promise((resolve, reject) => {
		...
		try {
			...
			shell.exec(`git clone ${MAIN_REPO_GIT} ${name}`, { silent: true }, (code, stdout, stderr) => {
				if (code !== 0) {
					cloneSpin.fail(stderr);
					reject(stderr);
					process.exit();
                }
                const removeGitOrigin = shell.exec('git remote remove origin',{ silent: true })
				cloneSpin.succeed(`Cloned 'name' successfully`);
				shell.cd(name);
				cloneSpin.succeed(`moved to /${name}`);
				const renameSpin = ora(`Renaming ${name} using ${chalk.yellow(`npx react-native-rename ${name}`)} ...`).start();
				shell.exec(`npx react-native-rename ${name}`, { silent: true }, (code, stdout, stderr) => {
					if (code !== 0) {
