...
async function init(name, options) {
	...
	try {
		...
		*/
		...
		shell.exec(resCheck.command, { silent: true }, (code, stdout, stderr) => {
			if (code !== 0) {
			...
			if (os.platform() === 'darwin') {
				...
				shell.exec(`pod install`, { silent: true }, async (code, stdout, stderr) => {
					if (code !== 0) {
					...
					shell.exec(`code .`, { silent: true }, (code, stdout, stderr) => {
						if (code !== 0) {
							return;
						}
