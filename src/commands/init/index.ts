...
export async function init(name: any, options: any) {
  ...
  try {
    ...
    const spinModules = ora(`Installing node_modules using ${resCheck.pkg}`).start();
    shell.exec(resCheck.command, { silent: true }, (code: number, stdout: any, stderr: any) => {
      if (code !== 0) {
      ...
      if (os.platform() === 'darwin') {
        ...
        shell.exec(`pod install`, { silent: true }, async (code: number, stdout: any, stderr: any) => {
          if (code !== 0) {
          ...
          shell.exec(`code .`, { silent: true }, (code: number, stdout: any, stderr: any) => {
            if (code !== 0) {
              return;
            }
             ...
             */
            success(` - Congrats, successfully cloned and installed all modules by rigel into ${name} 🎉 🎉 🎉`);
